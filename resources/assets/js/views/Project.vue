<template>
    <div class="project" v-if="project !== null">
        <!-- Hero Information -->
        <themed-section :color="project.hero_color" :image="project.hero_image" :isDark="Boolean(project.is_dark)"
                        :class="['project-header', 'align-center', 'no-padding']">
...
<script>
    ...
    export default {
        ...
        methods: {
            fetchData() {
                apiManiak
                    .getProjectBySlug(this.$props.slug)
                    .then(response => {
                        this.updateProject(response)
                        apiManiak
                            .getProjectNext(this.project.id)
